...
(this will terminate the print)."))
    ...
    def logdata_file(self):
        sprint = False
        last_t = 0
        self.status.logstop = False
        self.log(_("Devansh Modi: DataLog Started - Check -> /Data_Collection"))
        _temps_ = parse_temperature_report(self.tempreadings)
        #self.log(_(pprint.pprint(_temps_)))
        while not self.status.logstop:
         if(self.p.printing or self.p.paused or self.sdprinting):
                 last_t = time.time()

                 t_r = float("{0:.2f}".format(time.time() - self.status.logTime))
                 self.logdata_file_(t_r)
                 sprint = True
         elif(sprint):
            t_s = float("{0:.2f}".format(time.time() - last_t))
            if(t_s <= 60):
             self.logdata_file_(t_s)
            else: self.status.logstop = True

         else: self.log(_("Else Condition runs here."))

         time.sleep(0.25)

        self.log(_("Devansh Modi: DataLog Ended - Check -> /Data_Collection"))

    def logdata_file_(self, value):
          temps_ = parse_temperature_report(self.tempreadings)
          if "T0" in temps_ and temps_["T0"][0]:
                hotend_temp = float(temps_["T0"][0])
          elif "T" in temps_ and temps_["T"][0]:
                hotend_temp = float(temps_["T"][0])
          else:
                hotend_temp = None

          if "B" in temps_ and temps_["B"][0]:
                bed_temp = float(temps_["B"][0])
          else:
                bed_temp = None
          ...
          for h8 in range(0,len(fName)):
           if (fName[h8] == '.' and fName[h8+1:]=="gcode"):
               break
          ...
          fName = fName[h9+1:h8]
          with open('Data_Collection/DataExport_'+fName+'.txt', 'a') as the_file:
              if self.status.first:

               the_file.write(str('Time'))
               the_file.write(str('\t'))
               if "T0" in temps_ and temps_["T0"][0]:
                the_file.write(str('T0'))
               elif "T" in temps_ and temps_["T"][0]:
                the_file.write(str('T'))

               if "T1" in temps_ and temps_["T1"][0]:
                the_file.write(str('\t'))
                the_file.write(str('T1'))

               if "A1" in temps_ and temps_["A1"][0]:
                the_file.write(str('\t'))
                the_file.write(str('A1'))

               the_file.write(str('\t'))
               the_file.write(str('Bed'))
               the_file.write(str('\n'))

              self.status.first = False

              the_file.write(str(value))
              the_file.write(str('\t'))
              the_file.write(str(hotend_temp))

              if "T1" in temps_ and temps_["T1"][0]:
                the_file.write(str('\t'))
                the_file.write(str(float(temps_["T1"][0])))

              if "A1" in temps_ and temps_["A1"][0]:
                the_file.write(str('\t'))
                the_file.write(str(float(temps_["A1"][0])))


              the_file.write(str('\t'))
              the_file.write(str(bed_temp))
              the_file.write(str('\n'))

    def startcb(self, resuming = False):
        self.starttime = time.time()
        self.logdata = threading.Thread(target = self.logdata_file)

        if not resuming:
         self.status.logTime = self.starttime
        if resuming:
            self.log(_("Print resumed at: %s") % format_time(self.starttime))
        else:
            #self.log(_("Print YOYO BOY at: %s") % format_time(self.starttime))
